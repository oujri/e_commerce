...
def search(request):
    ...
    if request.method == 'POST':
        ...
        products_results = Product.objects.filter(Q(name__icontains=word) | Q(tags__name__icontains=word) | Q(supplier__user__first_name__icontains=word) | Q(supplier__user__last_name__icontains=word))
        print(is_pro)
        if is_pro == 'pro':
...
def products(request, type_search="all", id_search="0"):
    ...
    if type_search == "all":
        ...
        supplier = Profil.objects.filter(pk=id_search)
        if supplier.exists():
            products_results = Product.objects.filter(supplier__id__exact=id_search)
            message = "shop by supplier : {} {}".format(supplier[0].user.last_name, supplier[0].user.first_name)
            name = "{} {}".format(supplier[0].user.last_name, supplier[0].user.first_name)
        else:
            found = False
        ...
        if sub_categories_s.exists():
            products_results = Product.objects.filter(cat__category_two_id__exact=id_search)
            message = "shop by : {}".format(sub_categories_s[0].name)
            name = sub_categories_s[0].name
            category = sub_categories_s[0]
        else:
        ...
        if sub_categories_s.exists():
            products_results = Product.objects.filter(cat__category_two__category_one_id__exact=id_search)
            message = "shop by : {}".format(sub_categories_s[0].name)
            name = sub_categories_s[0].name
            category = sub_categories_s[0]
        else:
        ...
        if categories_s.exists():
            products_results = Product.objects.filter(cat__category_two__category_one__category_id__exact=id_search)
...
def display_product(request, id_product="0", message_success=None):
    ...
    if (timezone.localtime(timezone.now()).date() - product.date_add).days < 30:
        ...
        new = False
    # ----------- Menu Categories
    menu_categories = product.cat.category_two.category_one.commercescategorytwo_set.values('id').annotate(number=Count('commercescategory__product__id'))
...
def contact_supplier(request, id_product="None"):
    ...
    if request.method == "POST":
        ...
        message = request.POST.get("message")
        message = "Hello Mr {} {},<br/>Mr. {} contacts you about {} of {}, and here's his message : <b/>{}"\
            .format(product.supplier.user.last_name, product.supplier.user.first_name, name, quantity, product.name, message)
        MessageSupplier.objects.create(name=name, email=email, tel=tel, message=message, supplier=product.supplier)
        send_mail(
            'ESPR Order',
            message,
            'admin@socifly.com',
            ['elkhaouakarim@gmail.com'],
            fail_silently=False,
...
def sitemap(request):
    return render(request, 'ecommerce/sitemap.html', {
        'brands': Brand.objects.all()
    })
